...
async def get_response(start, end, max_concurrent_requests=5):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [safe_fetch(session, url, semaphore) for url in list(date_generator(start_date, end_date))]
        ...
        for articles in responses:
            for article in articles.get('result'):
                ...
                published_dt = datetime.fromisoformat(article.get('published_dt'))
                timestamp = int(
                    published_dt.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc).timestamp())
                parse_tasks.append(await scrape(session, url))
                ...
                article_metadata.append({
                    ...
                    'url': url,
                    'published_dt': published_dt.strftime('%Y-%m-%d'),
                    'published_timestamp': timestamp,
                    'tags': tags + rubrics,
                    'meta_description': article.get('meta_description'),
                    'currency_curs': history_curs.get(f"{timestamp}")
                })
                print(published_dt)
    ...
    try:
        async with semaphore, session.get(url) as response:
            if response.status == 200:
                ...
                # Обработка неудачных статусов HTTP
                return None
